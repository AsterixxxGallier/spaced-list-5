...
impl<S: Spacing, T> Skeleton<NestedRange, S, T> {
    pub(crate) fn push(this: Rc<RefCell<Self>>, distance: S, span: S, element: T) -> EphemeralPosition<NestedRange, S, T> {
    ...
    pub(crate) fn insert(this: Rc<RefCell<Self>>, position: S, span: S, element: T) -> EphemeralPosition<NestedRange, S, T> {
        ...
        match BoundType::of(result.index.try_into().unwrap()) {
            BoundType::Start => {
                ...
                // TODO check if less than or equals needs to be used in other places (notably similar assert statements)
                assert!(position + span <= result.position + outer_span,
