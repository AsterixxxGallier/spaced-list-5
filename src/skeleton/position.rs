...
impl<S: Spacing, T> EphemeralPosition<ClosedRange, S, T> {
...
impl<S: Spacing, T> EphemeralPosition<OpenNestedRange, S, T> {
    pub(crate) fn bound_type(&self) -> BoundType {
        BoundType::of(self.index.try_into().unwrap())
    }
            ...
            BoundType::Start => {
                if self.index + 1 < self.skeleton.borrow().elements.len() {
                    let end = Self::new(
                        self.skeleton.clone(),
                        self.index + 1,
                        self.position + self.span().unwrap());
                    (self, Some(end))
                } else {
                    (self, None)
                }
...
impl<S: Spacing, T> ElementRef<ClosedRange, S, T> {
    pub fn borrow(&self) -> Ref<T> {
...
macro_rules! position {
    ($name:ident; <Kind, S: Spacing$(, $T:ident)?>; $type:ty; $skeleton:ty) => {
        ...
        impl<S: Spacing$(, $T)?> $name<ClosedRange, S$(, $T)?> {
                    ...
                    BoundType::Start => {
                        ...
                        if ephemeral.index + 1 < ephemeral.skeleton.borrow().elements.len() {
                            let end = Self::new(
                                self.skeleton.clone(),
                                self.index + 1,
                                self.position + ephemeral.span().unwrap());
                            (self, Some(end))
                        } else {
                            (self, None)
                        }
