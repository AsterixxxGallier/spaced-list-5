...
macro_rules! checked {
    (shallow, $cmp:tt, $target:ident, $condition:ident ($($args:tt),*); $skeleton:ident) => {
        {
            ...
            after_loop!(shallow, $cmp, $target, $condition ($($args),*); $skeleton, degree, index, position)
        }
    };
    (deep, $cmp:tt, $target:ident, $condition:ident ($($args:tt),*); $skeleton:ident) => {
        {
            ...
            after_loop!(deep, $cmp, $target, $condition ($($args),*); skeleton, degree, index, position)
        }
    };
}
...
macro_rules! traversal_function_body {
    ($skeleton:ident; $depth:ident; $cmp:tt $target:ident with $condition:ident ($($args:tt),*)) => {
        if $skeleton.borrow().elements.is_empty() {
            ...
            if $condition!(0, $skeleton.clone(), $($args),*) && $skeleton.borrow().offset $cmp $target {
                Some(EphemeralPosition::new($skeleton.clone(), 0, $skeleton.borrow().offset))
            } else {
                None
            }
        } else {
            trivial_results!($cmp, $target, $condition ($($args),*); $skeleton);
            checked!($depth, $cmp, $target, $condition ($($args),*); $skeleton)
        }
    };
}
