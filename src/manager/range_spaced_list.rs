...
pub struct RangeSpacedListManager<S: Spacing, T> {
    list: RangeSpacedList<S, T>,
    locks: RangeSpacedListLocks,
}

impl<S: Spacing, T> RangeSpacedListManager<S, T> {
    fn new(list: RangeSpacedList<S, T>) -> Rc<RefCell<Self>> {
        Rc::new(RefCell::new(Self {
            list,
            locks: RangeSpacedListLocks::default(),
        }))
    }
